...
# Override compressor if COMPRESSOR env var is set
...
class Hyperparameters:
    ...
    rope_frac = float(os.environ.get("ROPE_FRAC", 0.25))  # fraction of head_dim to apply RoPE to
    num_xsa_heads = int(os.environ.get("NUM_XSA_HEADS", 4))  # number of exclusive self-attention heads
    ...
    gptq_calib_steps = int(os.environ.get("GPTQ_CALIB_STEPS", 32))
...
_last_block_idx = int(os.environ.get("NUM_LAYERS", 10)) - 1
FP16_KEEP_NAME_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get("FP16_KEEP_NAME_PATTERNS", f"tok_emb,blocks.{_last_block_idx}.attn.c_k").split(",")
    if pattern
)
...
def gptq_quantize_weight(W: Tensor, H: Tensor, clip_range: int = 31, block_size: int = 128, percdamp: float = 0.01) -> tuple[Tensor, Tensor]:
    """GPTQ: Hessian-calibrated quantization for a single weight matrix.
    W: (out_features, in_features), H: (in_features, in_features) = X^T X / n
    Returns: (quantized_int8, per_row_scale_fp16)
    """
    W = W.float().clone()
    rows, cols = W.shape

    # Compute per-row scale upfront (used for all quantization)
    ...
    H += damp * torch.eye(cols, device=H.device, dtype=H.dtype)
    try:
        Hinv = torch.cholesky_inverse(torch.linalg.cholesky(H))
    except Exception:
        return quantize_intN_per_row(W, clip_range=clip_range)
    ...
    for i1 in range(0, cols, block_size):
        ...
        Hinv_diag = torch.diag(Hinv[i1:i2, i1:i2]).clamp_min(1e-12)

        for j in range(i2 - i1):
            w_col = W_block[:, j]
            q_col = torch.clamp(torch.round(w_col / scale), -(clip_range + 1), clip_range)
            Q[:, i1 + j] = q_col.to(torch.int8)
            err = (w_col - q_col * scale) / Hinv_diag[j]
            # Distribute error to remaining columns in block
            if j + 1 < i2 - i1:
                W_block[:, j + 1:] -= err.unsqueeze(1) * Hinv[i1 + j, i1 + j + 1:i2].unsqueeze(0)
        # Distribute block error to remaining columns
        if i2 < cols:
            block_err = W[:, i1:i2] - Q[:, i1:i2].float() * scale.unsqueeze(1)
            W[:, i2:] -= block_err @ Hinv[i1:i2, i2:] / Hinv_diag.unsqueeze(1)

    return Q, scale.to(torch.float16)


# Storage for GPTQ calibration Hessians
_gptq_hessians: dict[str, tuple[Tensor, int]] = {}
_gptq_hooks: list = []

def register_gptq_hooks(model: nn.Module) -> None:
    """Register forward hooks to collect Hessian approximations for GPTQ."""
    _gptq_hessians.clear()
    _gptq_hooks.clear()
    ...
    def make_hook(name: str):
        def hook_fn(module, input, output):
            ...
            x = x.float()
            H = x.T @ x
            if name not in _gptq_hessians:
                _gptq_hessians[name] = (H, x.shape[0])
            else:
                prev_H, prev_n = _gptq_hessians[name]
                _gptq_hessians[name] = (prev_H + H, prev_n + x.shape[0])
        return hook_fn

    for name, module in model.named_modules():
        if isinstance(module, CastedLinear) and module.weight.ndim == 2 and module.weight.numel() > 65536:
            h = module.register_forward_hook(make_hook(name))
            _gptq_hooks.append(h)

def remove_gptq_hooks() -> None:
    for h in _gptq_hooks:
        h.remove()
    _gptq_hooks.clear()

def get_gptq_hessian(name: str) -> Tensor | None:
    if name in _gptq_hessians:
        H, n = _gptq_hessians[name]
        return H / n
    return None


def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], use_gptq: bool = False, model: nn.Module | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            H = get_gptq_hessian(name) if use_gptq else None
...
def apply_late_qat(model: nn.Module) -> None:
    """Apply STE fake-quantization to large weight matrices in-place during forward."""
    for name, param in model.named_parameters():
        if param.ndim == 2 and param.numel() > 65536:
            ...
            if cat == "mlp":
                ...
                continue
            param.data = STEQuantize.apply(param.data, clip)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
                 qk_gain_init: float, rope_frac: float = 0.25, num_xsa_heads: int = 0):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.num_xsa_heads = num_xsa_heads
        # Partial RoPE: only apply to a fraction of the head dimensions
    ...
    def forward(self, x: Tensor, q_delta=None, v_delta=None) -> Tensor:
        ...
        # XSA: exclusive self-attention for first num_xsa_heads heads
        if self.num_xsa_heads > 0 and self.num_xsa_heads < self.num_heads:
            q_std = q[:, self.num_xsa_heads:]
            k_std = k
            v_std = v
            y_std = F.scaled_dot_product_attention(
                q_std, k_std, v_std, attn_mask=None, is_causal=True,
                enable_gqa=(self.num_kv_heads != self.num_heads),
            )
            # XSA heads: subtract the self-attention component
            q_xsa = q[:, :self.num_xsa_heads]
            # For XSA, compute normal causal attention, then subtract value at self-position
            kv_group_size = self.num_heads // self.num_kv_heads
            xsa_kv_groups = min(self.num_xsa_heads, self.num_kv_heads)
            k_xsa = k[:, :xsa_kv_groups]
            v_xsa = v[:, :xsa_kv_groups]
            y_xsa_raw = F.scaled_dot_product_attention(
                q_xsa, k_xsa, v_xsa, attn_mask=None, is_causal=True,
                enable_gqa=(xsa_kv_groups != self.num_xsa_heads),
            )
            # Subtract value at the current position (exclusive = no self-attend)
            # v_self shape: (bsz, kv_heads, seqlen, head_dim) -> expand for xsa heads
            v_self = v_xsa.repeat_interleave(self.num_xsa_heads // xsa_kv_groups, dim=1)
            y_xsa = y_xsa_raw - v_self
            y = torch.cat([y_xsa, y_std], dim=1)
        else:
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float, rope_base: float,
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                         rope_frac=rope_frac, num_xsa_heads=num_xsa_heads)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        return F.cross_entropy(x.float(), target_ids.reshape(-1), reduction="mean")
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        for epoch in range(args.ttt_epochs):
            for ci in range(max_nc):
                ...
                # Score on first epoch only (avoid double-counting)
                if epoch == 0:
                    if needs_train:
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    if needs_train:
                        ...
                        per_doc = ptl[:, chunk_stats[2]:chunk_stats[2] + chunk_size].mean(dim=-1)
                        cur_opt.zero_grad()
                        (per_doc * mask).sum().backward()
                        cur_opt.step()
                else:
                    # Later epochs: train only, no scoring
                    if needs_train:
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            ptl = base_model(x, y, lora=cur_lora)
                        mask = torch.tensor([float(ci < num_chunks[b] - 1) for b in range(bsz)], device=device)
        ...
        if rank == 0 and (bi // batch_size) % 10 == 0:
            ...
            running_bpb = (loss_sum.item() / math.log(2.0)) / max(byte_sum.item(), 1) if token_count.item() > 0 else 0
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_xsa_heads=args.num_xsa_heads,
    ).to(device).bfloat16()
    ...
    while True:
        ...
        # Late QAT: apply STE fake-quantization in the last late_qat_frac of training
        if args.late_qat_frac > 0 and scale < args.late_qat_frac:
            with torch.no_grad():
                apply_late_qat(base_model)

        # (GPTQ calibration runs post-training in a separate loop)
    ...
    # GPTQ calibration: run forward passes on training data to collect Hessians
    if args.use_gptq:
        register_gptq_hooks(base_model)
        calib_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        base_model.eval()
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
            for calib_step in range(args.gptq_calib_steps):
                cx, cy = calib_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                _ = base_model(cx, cy)
        remove_gptq_hooks()
        log0(f"gptq:calibration_complete layers:{len(_gptq_hessians)} steps:{args.gptq_calib_steps}")
        base_model.train()
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn", "bigram"}, use_gptq=args.use_gptq, model=base_model)
    ...
    if _COMPRESSOR == "zstd":
        ...
        quant_blob = _lzma_mod.compress(quant_raw, preset=9 | _lzma_mod.PRESET_EXTREME)
    else:
    ...
    if args.use_ttt:
        ...
        log0(f"ttt:starting rank={rank} lora_rank={args.ttt_lora_rank} epochs={args.ttt_epochs} chunk={args.ttt_chunk_size}")
        ...
        log0(f"ttt_eval_exact val_loss:{ttt_loss:.8f} val_bpb:{ttt_bpb:.8f}")
